...
class SimpleRnnlm:
    ...
    def __init__(self, vocab_size, wordvec_size, hidden_size):
        ...
        rnn_Wx = (rn(D, H) / np.sqrt(D)).astype(np.float32) # Xavierの初期値
        rnn_Wh = (rn(H, H) / np.sqrt(H)).astype(np.float32) # Xavierの初期値
        rnn_b = np.zeros(H).astype(np.float32)
        affine_W = (rn(H, V) / np.sqrt(H)).astype(np.float32) # Xavierの初期値
        affine_b = np.zeros(V).astype(np.float32)

        # Timeレイヤの作成
        self.layers = [
            TimeEmbedding(embed_W),
            TimeRNN(rnn_Wx, rnn_Wh, rnn_b, statefull=True), # 順伝搬の隠れベクトルを引き継ぐ
